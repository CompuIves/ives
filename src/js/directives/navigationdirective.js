  ...
  function(ColorService) {
    return {
      ...
      link: function(scope, element, attr, ctrl) {
        ...
        $(element).append(
          "<span class='ultradarkcolor name'>Ives van Hoorne</span>"
        );
        ...
        items.forEach(function(item) {
          var li = $("<li></li>");
          var icon = $(
            "<i class='ultradarkcolor fa fa-" + item.icon + "'></i>"
          );
          $(element).append(li);
        ...
        var findActiveRecursive = function(updateColor) {
          ...
          for (var i = 0; i < items.length; i++) {
            if (
              windowTop <
              items[i].element.offset().top - $(window).height() / 4
            ) {
              if (
                !activeItem ||
                activeItem.navitem[0] != items[i - 1].navitem[0] ||
                updateColor
              ) {
                if (activeItem) {
        ...
        $(window).scroll(function() {
          if (colorsloaded) findActiveRecursive();
        });
      }
    };
  }
