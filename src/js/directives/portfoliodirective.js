    ...
    return {
      ...
      link: function(scope, element, attr, ctrl) {
        ...
        $(window).resize(function() {
          if (scope.item) buildElement(JSON.parse(scope.item));
        });
        ...
        function buildElement(item) {
          ...
          if (item.front.title) {
            fronttitle = $(
              "<div class='fronttitle'>" + item.front.title + "</div>"
            );
          ...
          title.css({
            color: ColorService.rgbToString(
              ColorService.lighten(ColorService.stringToRGB(item.color), 80)
            )
          });
          ...
          if (!item.small) {
            ...
            var calendartext = $(
              "<div class='text'>" + item.calendar + "</div>"
            );
