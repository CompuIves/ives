app.factory("ColorService", function() {
  ...
  function rgbToString(rgb, a) {
    ...
    if (rgb.a || a) newRgb += ", " + (rgb.a ? rgb.a : a);
    newRgb += ")";
    return newRgb;
  }
  ...
  function averageColors(rgbx, rgby) {
    ...
    newRgb.r = Math.max(
      0,
      loopNumber(Math.floor((rgbx.r + rgby.r) / 2), 0, 255)
    );
    newRgb.g = Math.max(
      0,
      loopNumber(Math.floor((rgbx.g + rgby.g) / 2), 0, 255)
    );
    newRgb.b = Math.max(
      0,
      loopNumber(Math.floor((rgbx.b + rgby.b) / 2), 0, 255)
    );
    return newRgb;
  }
